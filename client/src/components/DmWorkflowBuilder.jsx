...
const AudioLibraryModal = ({ onSelect, onClose }) => {
  ...
  const deleteAudio = async (filename) => {
    ...
    await fetch(
      `http://localhost:3000/api/bot/audios/${encodeURIComponent(filename)}`,
      { method: "DELETE" },
    );
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md mx-4 shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {audios.map((a) => (
            <div
              key={a.filename}
              className="flex items-center gap-3 p-3 bg-slate-800 rounded-xl border border-slate-700 hover:border-purple-500/40 transition-colors group"
            >
              ...
              <audio
                src={`http://localhost:3000/api/bot/audios/file/${encodeURIComponent(a.filename)}`}
                controls
                className="h-7 w-28 shrink-0"
              />
              <button
                onClick={() => onSelect(a)}
                title="Usar este áudio"
              ...
              <button
                onClick={() => deleteAudio(a.filename)}
                title="Apagar"
          ...
          {isText && (
            <div>
              ...
              <p className="text-xs text-slate-600 mt-1">
                ...
                para spintax (variações aleatórias). Ex:{" "}
                <code className="text-slate-400">Olá! | Oi! | E aí!</code>
              </p>
            </div>
          )}
...
const DmWorkflowBuilder = ({ steps = [], onChange }) => {
  const addStep = (type) => {
    onChange([
      ...steps,
      { type, text: "", audioBlob: null, audioUrl: null, audioFilename: null },
    ]);
  };
