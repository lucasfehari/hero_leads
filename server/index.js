  ...
  try {
    ...
    try {
      await page.waitForSelector(
        'svg[aria-label="Home"], svg[aria-label="Página inicial"], svg[aria-label="Início"]',
        { timeout: 300000 },
      );
    } catch (e) {
      await browser.close();
    ...
    try {
      const data = await page.evaluate(() => {
        const img =
          ...
          document.querySelector('img[alt*="foto de perfil"]');
...
app.post("/api/profiles", (req, res) => {
  ...
  if (!name || !cookies)
    return res.status(400).json({ error: "Name and Cookies required" });
  try {
...
const BOT_AUDIOS_DIR = path.join(
  require("os").homedir(),
  ".browzebot",
  "uploads",
  "bot_audios",
);
if (!fs.existsSync(path.join(require("os").homedir(), ".browzebot", "uploads")))
  fs.mkdirSync(
    path.join(
      require("os").homedir({ recursive: true }),
      ".browzebot",
      "uploads",
    ),
  );
if (!fs.existsSync(BOT_AUDIOS_DIR))
  fs.mkdirSync(BOT_AUDIOS_DIR, { recursive: true });
...
app.post(
  "/api/bot/upload-audio",
  uploadBotAudio.single("audio"),
  (req, res) => {
    if (!req.file)
...
app.post("/api/bot/test-ai", async (req, res) => {
  ...
  if (!key)
    return res
      .status(400)
      .json({ success: false, error: "Chave não fornecida" });
  try {
...
app.post("/api/ai/generate-messages", async (req, res) => {
  ...
  if (!key)
    return res
      ...
      .json({
        success: false,
        error: "Chave OpenRouter não configurada. Vá em Global Settings.",
      });
  if (!leads || !Array.isArray(leads) || leads.length === 0)
    return res
      .status(400)
      .json({ success: false, error: "Nenhum lead fornecido." });
  if (!prompt || !prompt.trim())
    return res
      .status(400)
      .json({ success: false, error: "Prompt não fornecido." });
  ...
  const generateForLead = async (lead, idx) => {
    ...
    const leadContext = [
      ...
      rating
        ? `Avaliação no Google: ${rating}${reviews ? ` (${reviews} avaliações)` : ""}`
        : "",
    ]
      .filter(Boolean)
      .join("\n");
  ...
  for (let i = 0; i < leads.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(
      batch.map((lead, batchIdx) => generateForLead(lead, i + batchIdx)),
    );
...
app.get("/api/history/profiles", (req, res) => {
  try {
    res.json({ profiles: historyDb.getProfiles() });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post("/api/google-maps/start", async (req, res) => {
  try {
    ...
    broadcastLog(
      "Starting Google Maps Bot with queries: " + queries.join(" | "),
    );
...
app.get("/api/maps/leads", (req, res) => {
  try {
    ...
    res.json(
      mapsDb.getLeads({ page, limit, query, hasWebsite, minStars, minReviews }),
    );
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post("/api/maps/validate-whatsapp", async (req, res) => {
  try {
    ...
    if (!leads || !Array.isArray(leads))
      return res.status(400).json({ error: "Lista de leads requerida." });
    if (!waClient.isConnected)
      return res
        .status(400)
        .json({ error: "WhatsApp não está conectado para validar." });
    ...
    for (let lead of leads) {
      if (lead.phone) {
        ...
        if (
          countryCode &&
          !(
            num.startsWith(countryCode) && num.length >= countryCode.length + 10
          )
        ) {
          num = countryCode + num;
        }
...
const PROFILES_DATA_DIR = path.join(
  require("os").homedir(),
  ".browzebot",
  "../profiles_data",
);
...
app.post("/api/maps/profiles/login", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[System] Navigate to StackOverflow Login. Please click 'Log in with Google'.`,
    );

    // We don't automate login here, we just wait for the user to close the browser
    console.log(
      `[System] Waiting for user to close browser for profile ${name}...`,
    );
...
app.get("/api/whatsapp/sessions", (req, res) => {
  res.json({
    ...
    current: waClient.currentSession,
    isPoweredOn: !!waClient.client,
  });
});
...
app.post("/api/whatsapp/start", (req, res) => {
  try {
    ...
    if (!numbers || !msgs)
      return res.status(400).json({ error: "Numbers and messages required" });
...
app.post("/api/whatsapp/optout-config", (req, res) => {
  try {
    ...
    if (typeof autoReply === "boolean")
      waClient.optOutConfig.autoReply = autoReply;
    if (typeof replyMessage === "string")
      waClient.optOutConfig.replyMessage = replyMessage;
